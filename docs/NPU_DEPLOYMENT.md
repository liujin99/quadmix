...
```bash
# 8 线程并行下载（推荐）
python scripts/preprocess/download_essential_web.py --num-files 2000 --workers 8
...
HF_ENDPOINT=https://hf-mirror.com python scripts/preprocess/download_essential_web.py --num-files 2000 --workers 8
...
python scripts/preprocess/download_essential_web.py --num-files 2000 --use-hf-transfer
```
...
| 数据集 | 方法 | 预计耗时 | 适用场景 |
|--------|------|----------|----------|
| Essential-web (791GB) | 单线程（默认） | 4-8 小时 | 网络不稳定 |
| | 4 线程并行 | 1-2 小时 | 一般网络 |
...
| 文件 | 用途 |
|------|------|
| `scripts/runners/run_essential_web_v1.py` | 主入口，所有 CLI 参数在此 |
| `src/quadmix/pipeline/essential_proxy_runner.py` | 代理模型训练、token cache、多卡调度 |
| `scripts/preprocess/preprocess_essential_web_v1_sharded.py` | 数据预处理 |
| `scripts/preprocess/download_essential_web.py` | 原始数据下载 |
