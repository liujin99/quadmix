...
|--------|------|------|------|
| ~~P0~~ | ~~A1~~ | ~~质量分数方向反转~~ | ✅ 已修复 |
| ~~P1~~ | ~~A4~~ | ~~Proxy 训练 token 数太少~~ | ✅ 已确认 655M 可接受 |
...
| P2 | B2 | Bayesian Optimization | 待实施（根因明确后优先级降低） |
| ~~P3~~ | ~~A7,B5~~ | ~~缺少 BMK 变体 / 消融支持~~ | ✅ 已有 core_bmk_v2~v6 |
...
> **2026-07-03 更新**: C/D/E/F 系列性能优化项当前优先级降低。根因已明确（验证集目标不匹配），性能优化不会改善中训练效果。待 I 系列完成后视需要再实施。
...
> **2026-07-03 更新**: 显存优化已基本完成（bf16 + Flash Attention），剩余项优先级降低。
...
> **2026-07-03 更新**: 根因已明确（验证集目标与下游目标不匹配），G 系列大部分项优先级降级。Proxy R²=0.79 已够准，优化搜索/模型/集成不会解决根因问题。优先实施 I 系列（换验证集）。
...
|:---|:---|:---|:---|:---|:---|
| P0 | G1 | Bayesian Optimization | Spearman +0.1~0.2 | 中 | 待实施（根因明确后优先级降低） |
| P0 | G3 | Conformal Prediction | 置信区间保证 | 低 | 待实施（根因明确后优先级降低） |
| ~~P1~~ | ~~G7~~ | ~~增加实验到 2000+~~ | ~~R² +0.1~0.2~~ | ~~高（算力）~~ | ✅ 已完成（demo_run_full 用 3000 实验） |
| P1 | G4 | 多模型集成 | R² +0.05~0.1 | 中 | 待实施（根因明确后优先级降低） |
| P1 | G8 | 参数正交化 | 有效维度降低 | 中 | 待实施（根因明确后优先级降低） |
| P1 | G2 | 两阶段搜索 | Top-K Recall 提升 | 低 | 待实施（根因明确后优先级降低） |
| P2 | G6 | Active Learning | 信息增益最大化 | 中 | 待实施（根因明确后优先级降低） |
| P2 | G10 | 扩大验证集 | 标签噪声降低 | 中 | 待实施（根因明确后优先级降低） |
| P2 | G5 | Multi-Task / Stacking | 低 R² task 提升 | 中 | 待实施（根因明确后优先级降低） |
| P2 | G11 | Proxy 容量提升 | 推理 task 信号 | 高（算力） | 待实施（根因明确后优先级降低） |
| P2 | G9 | 多 Proxy 集成 | 标签噪声 √3 倍 | 高（算力） | 待实施（根因明确后优先级降低） |
| P3 | G13 | SHAP 敏感度分析 | 搜索效率 2-3x | 低 | 待实施（根因明确后优先级降低） |
| P3 | G12 | 非线性/交互特征 | 模型表达力 | 低 | 待实施（根因明确后优先级降低） |
...
## QuadMix 中训练效果改进方向 — 2026-07-03

### 背景

4 方法对比实验结果（OpenHermes 验证集 revalidate，R²=0.79）：

| 方法 | CORE Metric | Final Loss |
|---|---|---|
| QuadMix | 0.2714 | 2.182 |
| Random | 0.2638 | 2.279 |
| Quality (dclm) | 0.2714 | 1.610 |
| Quality (fineweb_edu) | 0.2737 | 1.991 |

域分布诊断结果：QuadMix 与 Quality(dclm) 域分布几乎相同（Top-3 集中度均 41.3%，HHI 差 1），CORE 也完全相同。Proxy 搜索收敛到了与 DCLM top-k 等效的解。

### 根因分析

**Proxy 优化的目标与下游目标不匹配。**

- Proxy 在 essential-web 子集上训练 → 在 OpenHermes（GPT-4 对话回复）上评估 val_loss
- LightGBM 学到：选高质量网页文本 → 对话续写 loss 低（R²=0.79）
- 搜索出 θ* 自然倾向于选"语言质量高"的文档 → 和 DCLM quality-top-k 选出同样的东西
- 但**对话续写能力 ≠ 推理能力**，所以 QuadMix 没有比 quality-top-k 更好

### 改进方向

#### I1. 换验证集为推理导向数据（高优先级）

- **问题**: OpenHermes 验证集让 proxy 学"选语言流畅的文本"，而非"选能提升推理的文本"
- **方案**: 用推理密集文本做 proxy 验证集
- **候选数据来源**:
  1. **GSM8K / MATH** 的 step-by-step 解答（数学推理）
  2. **LogiQA / ReClor** 的逻辑推理段落
  3. **代码 + docstring**（算法推理）
  4. **从 OpenHermes 1M 中筛选推理类对话**（最省事，不改架构只换数据分布）
- **预期效果**: Proxy 学到"选能提升推理的文本"，QuadMix 的域感知优势得以体现
- **实施成本**: 低（方案 4）~ 中（方案 1-3）

#### I2. 用 CORE BMK per-task 验证（中优先级）

- **方案**: 21 个独立 LightGBM 模型分别优化各 task loss，R²×σ 加权搜索
- **问题**: 部分 task R² 很低，加权后搜索信号弱
- **前置条件**: 需先解决 R² 不均匀问题（扩大低样本 task 的验证集、提升 proxy 模型容量）
- **实施成本**: 中

#### I3. 增加推理相关质量维度（低优先级）

- **问题**: 当前 5 个质量分（DCLM、fineweb_edu 等）都衡量语言质量，缺乏推理信号
- **方案**: 增加推理密度分数（用小模型打分：这段文本包含多少推理步骤）
- **预期效果**: 让 proxy 有推理信号可学
- **实施成本**: 高（需要训练/获取推理密度打分模型）

#### I4. 不改 proxy，改中训练策略（低优先级）

- **方案**:
  1. 课程学习：先训通用文本，再训推理密集文本
  2. 数据重复策略：推理相关域多 epoch
- **实施成本**: 低
- **风险**: 不解决数据选择问题，只是训练技巧
...
| 优先级 | 编号 | 改进 | 预期收益 | 实施成本 | 状态 |
|:---|:---|:---|:---|:---|:---|
| **P0** | I1 | 换验证集为推理导向数据 | 解决根因 | 低~中 | 待实施 |
| **P1** | I2 | CORE BMK per-task 验证 | 精细化搜索 | 中 | 待实施 |
| **P2** | I3 | 增加推理质量维度 | 额外信号 | 高 | 待实施 |
| **P2** | I4 | 改中训练策略 | 训练技巧 | 低 | 待实施 |
