"""
Analyze domain distribution from preprocessed parquet shards.

Reads the domain and doc_char_count columns from preprocessed output,
computes per-domain statistics, and produces a bar chart + JSON report.
...
from quadmix.constants import FDC_PREFIX_TO_DOMAIN
...
def _scan_shard(path: str, domain_col: str, char_count_col: str, num_domains: int) -> dict:
    cols_to_read = [domain_col]
    if char_count_col:
        cols_to_read.append(char_count_col)
    try:
    ...
    if char_count_col:
        chars = df[char_count_col].to_numpy(dtype=np.int64)
    else:
        chars = np.zeros(len(df), dtype=np.int64)
...
def main():
    p = argparse.ArgumentParser(description="Analyze domain distribution from preprocessed shards")
    ...
    char_count_col = schema.char_count_col
    ...
    if not paths:
        all_parquets = sorted(glob.glob(os.path.join(args.preprocessed_dir, "*.parquet")))
        if not all_parquets:
            print(f"Error: no .parquet files found in {args.preprocessed_dir}")
            return 1
        paths = all_parquets

    print(f"Scanning {len(paths)} shards from {args.preprocessed_dir}")
    print(f"  Schema: domain_col='{domain_col}', char_count_col={char_count_col}, num_domains={num_domains}")
    t0 = time.time()
    ...
    if len(paths) > 1:
        with ProcessPoolExecutor(max_workers=workers) as executor:
            futures = {executor.submit(_scan_shard, p, domain_col, char_count_col, num_domains): p for p in paths}
            done = 0
        ...
        r = _scan_shard(paths[0], domain_col, char_count_col, num_domains)
        if "error" not in r:
            total_counts += np.array(r["counts"], dtype=np.int64)
            total_chars += np.array(r["char_sums"], dtype=np.int64)
            total_docs += r["total_docs"]
    ...
    actual_num_domains = max(int(total_counts.argmax()) + 1, num_domains)
    if actual_num_domains > num_domains:
        extra_counts = np.zeros(actual_num_domains - num_domains, dtype=np.int64)
        total_counts = np.concatenate([total_counts, extra_counts])
        extra_chars = np.zeros(actual_num_domains - num_domains, dtype=np.int64)
        total_chars = np.concatenate([total_chars, extra_chars])
    ...
    print(f"\n{'=' * 90}")
    print(f"  Domain Distribution ({num_domains} domains, {len(paths)} shards)")
    ...
    fig, axes = plt.subplots(1, 2, figsize=(18, max(8, num_domains * 0.35)))

    sorted_names = [domain_names[int(d)] if int(d) < len(domain_names) else f"D{d}" for d in sorted_ids]
    ...
    axes[0].set_title("Domain Distribution (by doc count)")
    axes[0].invert_yaxis()
    ...
    axes[1].set_title("Domain Distribution (by token count)")
    axes[1].invert_yaxis()
    ...
    plt.tight_layout()
    fig_path = os.path.join(args.output_dir, "domain_distribution.png")
    ...
    if args.raw_dir:
        ...
        if raw_paths:
            print(f"\n{'=' * 90}")
            print(f"  Discarded FDC Prefix Analysis (Essential-Web specific)")
