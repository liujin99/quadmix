#!/usr/bin/env python3
"""
Prepare CORE-BMK v6 validation set for QuaDMix proxy model.

Changes from v5:
  - HellaSwag preprocessing fixed: removes [header]/[title]/[step]/[substeps] tags,
    adds activity_label prefix, capitalizes sentence boundaries (matches eval_bundle format)
  - MC_PER_SAMPLE_TASKS removed: arc_easy/arc_challenge/piqa/openbook_qa now use
    standard continuation_delimiter path, matching eval_bundle format 100%
    (v5 had per-sample branching that caused format inconsistency for ~16-51% of samples)
  - All other tasks unchanged from v5
...
Output:
  data/core_bmk_21tasks_v6_tokenized.pt
  data/core_bmk_21tasks_v6.parquet
"""
...
ALL_V6_TASKS = FULL_SEQ_TASKS + ANSWER_ONLY_TASKS
...
def clean_hellaswag_ctx(ctx, activity_label):
    """
    Clean HellaSwag context to match eval_bundle format.
    
    HellaSwag HF dataset contains structured tags from wikiHow and ActivityNet:
    - [header]: Topic title
    - [title]: Section title  
    - [step]: Action step
    - [substeps]: Sub-steps
    ...
    Transformation:
    - [header] X [title] Y -> X. Y (header gets period, title removed)
    - [title] -> . (sentence separator)
    - [step], [substeps] -> space (removed)
    - Other brackets like [www.xxx] -> space (removed)
    - activity_label prepended as prefix
    - First letter after '. ' capitalized
    """
    has_tags = bool(re.search(r'\[.*?\]', ctx))
    
    if has_tags:
        cleaned = re.sub(r'\[header\]\s*(.*?)\s*\[title\]\s*', r'\1. ', ctx)
        cleaned = re.sub(r'\s*\[title\]\s*', '. ', cleaned)
        cleaned = re.sub(r'\s*\[(step|substeps)\]\s*', ' ', cleaned)
    ...
    if activity_label:
        return f'{activity_label}: {cleaned}'
    return cleaned


def _convert_hellaswag(s):
    ctx = clean_hellaswag_ctx(s["ctx"], s.get("activity_label", ""))
    return {
        "query": ctx,
        "choices": s["endings"],
        "gold": int(s["label"]),
...
def main():
    parser = argparse.ArgumentParser(
        description="Create CORE-BMK v6 validation set for QuaDMix"
    )
    ...
    eb_count = len(ALL_V6_TASKS) - hf_count
    print(f"\nv6 data sources:")
    print(f"  HuggingFace tasks ({hf_count}): {list(HF_TASKS.keys())}")
    print(f"  Eval-bundle tasks ({eb_count}): {[t for t in ALL_V6_TASKS if t not in HF_TASKS]}")

    all_samples = []
    task_meta = []

    for label in ALL_V6_TASKS:
        if label not in task_map:
    ...
    output = {
        ...
        "metadata": {
            ...
            "source": f"CORE-BMK-{len(task_meta)}tasks-v6",
            "eval_bundle": args.eval_bundle,
            "num_samples_per_task": args.num_samples_per_task,
            "seed": seed,
            "description": (
                f"CORE-BMK v6: {len(task_meta)} tasks (all CORE benchmarks, deduped). "
                f"11 tasks loaded from HuggingFace (train+test+val merged), 10 from eval_bundle. "
                f"HellaSwag preprocessing fixed: [header]/[title]/[step]/[substeps] tags removed, "
                f"activity_label prefix added, sentence boundaries capitalized (matches eval_bundle format). "
    ...
    save_path = os.path.join(output_dir, "core_bmk_21tasks_v6_tokenized.pt")
    ...
    parquet_path = os.path.join(output_dir, "core_bmk_21tasks_v6.parquet")
