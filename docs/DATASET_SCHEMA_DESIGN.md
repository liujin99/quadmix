...
**v2.0 变更**：所有数据集（包括 Essential-Web）必须通过 YAML 配置。`DatasetSchema()` 无参构造会报错。示例配置位于 `configs/schema_essential_web.yaml`。
...
```yaml
# configs/schema_stem.yaml — STEM 数据集示例
...
```yaml
# configs/schema_essential_web.yaml — Essential-Web 示例数据集（默认示例）
domain_col: domain
domain_names:
...
用户: YAML + parquet 目录
           ↓
DatasetSchema.from_yaml(path)        ← 解析 YAML，得到列映射配置（唯一入口）
           ↓
ShardMetadataManager(dir, schema)    ← 按 schema 读取对应列（schema 必填）
           ↓
