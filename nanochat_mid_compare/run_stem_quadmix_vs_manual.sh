#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────
# QuadMix vs Manual Ratio: STEM Mid-Training Comparison
# ──────────────────────────────────────────────────────────────
#
# This script runs nanochat mid-training on two datasets:
#   1. QuadMix-selected subset from STEM data
#   2. Manual Ratio subset from STEM data (domain-proportional, token-count aligned)
#
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  QuadMix vs Manual Ratio — STEM Mid-Training"
echo "════════════════════════════════════════════════════════════"
echo ""
...
# Skip random baseline — only prepare quadmix + manual_ratio
...
stats['config'].update({
    ...
    'baselines': ['quadmix', 'manual_ratio'],
})
...
echo "  Minimum needed:  ${MIN_REQUIRED_GB}GB (2 trainings × ~20GB/ckpt)"
...
if [ "$AVAILABLE_GB" -lt "$MIN_REQUIRED_GB" ]; then
    ...
    echo "       MID_CHECKPOINTS_OUTPUT_DIR=/path/to/larger/disk bash nanochat_mid_compare/run_stem_quadmix_vs_manual.sh"
...
QUADMIX_TOKENS=$(STATS_FILE="$STATS_FILE" python3 -c "
import os, json
s = json.load(open(os.environ['STATS_FILE']))
...
echo ""
echo "╔══ Step 3b: Mid-training on Manual Ratio data ══╗"
echo ""
