#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────
# Random Baseline Only: STEM Mid-Training
# ──────────────────────────────────────────────────────────────
#
# This script runs nanochat mid-training on a single dataset:
#   1. Random subset from STEM data (token-budget aligned)
#
# No QuadMix or Manual Ratio data is prepared.
#
# Usage (after running QuaDMix pipeline on STEM data):
#   bash nanochat_mid_compare/run_stem_random_only.sh
#
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  Random Baseline Only — STEM Mid-Training"
echo "════════════════════════════════════════════════════════════"
echo ""
...
echo ""
echo "╔══ Step 1: Prepare STEM dataset (random only) ══╗"
echo ""
...
if [ "$AVAILABLE_GB" -lt "$MIN_REQUIRED_GB" ]; then
    ...
    echo "       MID_CHECKPOINTS_OUTPUT_DIR=/path/to/larger/disk bash nanochat_mid_compare/run_stem_random_only.sh"
...
RANDOM_TOKENS=$(STATS_FILE="$STATS_FILE" python3 -c "
import os, json
s = json.load(open(os.environ['STATS_FILE']))
if 'random' in s:
    print(s['random']['tokens'])
else:
    print('0')
")
...
echo ""
echo "╔══ Step 3: Mid-training on Random data ══╗"
echo ""
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  STEM Random-Only Experiment Complete!"
echo "════════════════════════════════════════════════════════════"
echo ""
