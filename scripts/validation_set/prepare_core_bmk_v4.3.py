#!/usr/bin/env python3
"""
Prepare CORE-BMK v4.3 validation set for QuaDMix proxy model.

Changes from v4.2:
  - piqa, arc_easy, arc_challenge moved from answer-only to full-sequence
  - These 3 tasks now use "\\nAnswer: " separator for natural Q&A format
  - Full-seq: 15 tasks, Answer-only: 6 tasks

Task classification:
  A. Full-sequence loss (15 tasks) — context+continuation = natural text:
     hellaswag_zeroshot, lambada_openai, winogrande, winograd, copa,
     jeopardy, boolq, squad, coqa, bigbench_language_identification,
     bigbench_qa_wikidata, openbook_qa, piqa, arc_easy, arc_challenge
  B. Answer-only loss (6 tasks) — context is Q+A/SFT/symbolic:
...
Usage:
  python scripts/validation_set/prepare_core_bmk_v4.3.py

Output:
  data/core_bmk_21tasks_v4.3_tokenized.pt
  data/core_bmk_21tasks_v4.3.parquet
"""
...
ALL_V43_TASKS = FULL_SEQ_TASKS + ANSWER_ONLY_TASKS

MC_EMBEDDED_TASKS = {"commonsense_qa", "agi_eval_lsat_ar", "bigbench_language_identification"}

MC_FULLSEQ_ANSWER_SEP_TASKS = {"piqa", "arc_easy", "arc_challenge"}
    ...
    if task_type == "multiple_choice":
        for item in items:
            ...
            if not question.strip():
                continue
            if is_full_seq:
                if task_label in MC_FULLSEQ_ANSWER_SEP_TASKS:
                    sep = "\nAnswer: "
                else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Create CORE-BMK v4.3 validation set for QuaDMix"
    )
    ...
    print(f"\nFiltering to {len(ALL_V43_TASKS)} tasks (deduped hellaswag)...")
    ...
    output = {
        ...
        "metadata": {
            ...
            "source": f"CORE-BMK-{len(task_meta)}tasks-v4.3",
            "eval_bundle": args.eval_bundle,
            "num_samples_per_task": args.num_samples_per_task,
            "seed": seed,
            "description": (
                f"CORE-BMK v4.3: {len(task_meta)} tasks (all CORE benchmarks, deduped). "
    ...
    save_path = os.path.join(output_dir, "core_bmk_21tasks_v4.3_tokenized.pt")
    ...
    parquet_path = os.path.join(output_dir, "core_bmk_21tasks_v4.3.parquet")
